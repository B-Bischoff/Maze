...
public class Maze : MonoBehaviour
{
    ...
    void CreateMaze(int width, int height)
    {
        ...
        // Creating ground
        Vector3 groundPos = new Vector3(width / 2, 0, height / 2);
        Vector3 groundScale = new Vector3(width / 10, 1, height / 10);
    ...
    MazeCell CheckNeighbours(MazeCell cell, int height, int width)
    {
        ...
        if (cell.y > 0 && _maze[cell.y - 1, cell.x].visited == 0)
            list.Add(_maze[cell.y - 1, cell.x]);
        if (cell.y < height - 1 && _maze[cell.y + 1, cell.x].visited == 0)
            list.Add(_maze[cell.y + 1, cell.x]);
        if (cell.x > 0 && _maze[cell.y, cell.x - 1].visited == 0)
            list.Add(_maze[cell.y, cell.x - 1]);
        if (cell.x < width - 1 && _maze[cell.y, cell.x + 1].visited == 0)
            list.Add(_maze[cell.y, cell.x + 1]);
        if (list.Count == 0)
            return null;
